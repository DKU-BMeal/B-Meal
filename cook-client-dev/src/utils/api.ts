...
export async function personalizeRecipe(recipe: any, profile: any) {
  return apiCall(
    "/ai/personalize",
    { method: "POST", body: JSON.stringify({ recipe, profile }) },
    true
  );
}
...
export async function getRecipeRecommendations(
  profile: any,
  options: { exclude?: string[]; userContext?: string } = {}
) {
  return apiCall(
    "/ai/recommend",
    { method: "POST", body: JSON.stringify({ profile, ...options }) },
    true
  );
}
...
// FRIDGE SCAN
// ===============================
export async function scanFridgeImage(formData: FormData) {
  const token = getAuthToken();
  const controller = new AbortController();
  const timeoutId = setTimeout(() => controller.abort(), 60000);
  try {
    const res = await fetch(`${API_BASE_URL}/ingredients/fridge-scan`, {
      method: "POST",
      headers: token ? { Authorization: `Bearer ${token}` } : undefined,
      body: formData,
    ...
    if (e.name === "AbortError") throw new Error("분석 시간이 초과됐어요 (60초). 더 작은 사진으로 다시 시도해보세요.");
    throw e;
  } finally {
