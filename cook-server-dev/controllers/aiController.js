...
// ★ 레시피 프로필 맞춤화 API
// ===============================================
export const personalizeRecipe = async (req, res) => {
  try {
    const { recipe, profile } = req.body;

    if (!recipe || !profile) {
      return res.status(400).json({ error: "recipe and profile are required" });
    }

    const result = await askGPTPersonalize(recipe, profile);
    ...
    res.status(500).json({ error: "레시피 맞춤화 중 오류가 발생했습니다." });
  }
};
...
// ★ 요리 추천 목록 API
// ===============================================
export const recommendDishes = async (req, res) => {
  try {
    const { profile, exclude = [], userContext = "" } = req.body;
    const result = await askGPTRecommend(profile, { exclude, userContext });
    ...
    res.status(500).json({ error: "요리 추천 중 오류가 발생했습니다." });
  }
};
