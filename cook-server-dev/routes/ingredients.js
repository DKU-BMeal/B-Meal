...
import { scanFridgeImage } from '../services/aiService.js';
...
// ============================================
// 냉장고 사진 스캔 (GPT-4o-mini Vision)
// ============================================
const uploadDir = path.join(process.cwd(), 'uploads/fridge');
if (!fs.existsSync(uploadDir)) fs.mkdirSync(uploadDir, { recursive: true });

const fridgeUpload = multer({ dest: uploadDir, limits: { fileSize: 10 * 1024 * 1024 } });

router.post('/fridge-scan', fridgeUpload.single('image'), async (req, res) => {
  try {
    ...
    console.log('[FridgeScan] 이미지 크기:', (base64.length / 1024).toFixed(0), 'KB (base64)');

    const timeout = new Promise((_, reject) =>
      setTimeout(() => reject(new Error('GPT 응답 시간 초과')), 55000)
    );
    const result = await Promise.race([scanFridgeImage(base64), timeout]);

    console.log('[FridgeScan] 결과:', JSON.stringify(result).slice(0, 200));
    res.json({ success: true, result });
  } catch (err) {
    console.error('[FridgeScan] Error:', err.message);
    res.status(500).json({ error: err.message || '냉장고 분석 중 오류가 발생했습니다.' });
  }
});
